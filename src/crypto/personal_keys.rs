...
impl fmt::Display for PersonalKeys {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f, "public {}", to_hex_string(self.pk.as_ref()))
    }
}
...
impl PersonalKeys {
    ...
    pub fn sign(&self, message: &str, password: &str) -> Result<sign::Signature, Error> {
        ...
        let signature = sign::sign_detached(message.as_bytes(), &sk);
    ...
    fn decrypt_sign_key(&self, passwd: &[u8]) -> Result<sign::SecretKey, Error> {
        ...
        let sk_bytes =
            secretbox::open(&self.encrypted_secret, &self.nonce, &encryption_key).unwrap();
