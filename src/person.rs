...
impl Person {
	...
	pub fn sign_msg(&self, message: &str, password: &str) -> Result<crypto::sign::Signature, Error> {
...
pub fn create_person(name: &str, password: &str) -> Person {
	Person { name: name.to_string(), keys: PersonalKeys::new(&password) }
}
